...
export async function setFiterBy(filterBy) {
    console.log('filter by in action', filterBy)
    try {
        return store.dispatch(setFilterBy(filterBy))
    } catch (err) {
        console.log('Cannot edit filter by object', err)
        throw err
    }
}
